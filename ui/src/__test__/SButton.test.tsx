...
describe('SButton Component', () => {
	...
	it('applies the correct classes based on size prop', () => {
		...
		expect(container.firstChild).toHaveClass(
			'h-62pxr py-16pxr px-28pxr text-18pxr leading-30pxr'
		);
	});
	...
	it('applies the correct background color based on color prop', () => {
		...
		expect(screen.getByText('Colored Button')).toHaveClass(
	...
	it('applies the correct outline classes when outline prop is true', () => {
		...
		expect(screen.getByText('Outlined Button')).toHaveClass(
			'before:border-[#E30000] text-[#E30000]'
		);
	});
	...
	it('disables the button when disabled prop is true', () => {
		...
		expect(button).toHaveClass(
			'disabled:bg-Grey_Lighten-3 disabled:text-Grey_Default'
		);
	});
